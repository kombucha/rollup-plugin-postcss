...
var postcssLoader = {
  ...
  async process({
    ...
    const postcssOpts = _objectSpread({}, this.options.postcss, config.options, {
...
var stylusLoader = {
  ...
  async process({
    ...
    const style = stylus(code, _objectSpread({}, this.options, {
      filename: this.id,
      sourcemap: this.sourceMap && {}
    }));
...
var lessLoader = {
  ...
  async process({
    ...
    let {
      ...
      map,
      imports
    } = await pify(less.render.bind(less))(code, _objectSpread({}, this.options, {
      sourceMap: this.sourceMap && {},
      filename: this.id
    }));
...
var index = ((options = {}) => {
  ...
  return {
    ...
    async generateBundle(opts, bundle) {
      ...
      const getExtracted = () => {
        const fileName = typeof postcssLoaderOptions.extract === 'string' ? path.relative(dir, postcssLoaderOptions.extract) : `${path.basename(opts.file, path.extname(opts.file))}.css`;
        const concat = new Concat(true, fileName, '\n');
        const entries = Array.from(extracted.values());
        const {
          modules
        } = bundle[path.relative(dir, opts.file)];
      ...
      let {
        ...
        map,
        mapFileName
      } = getExtracted(); // Perform cssnano on the extracted file
